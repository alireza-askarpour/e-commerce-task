...
const SectionHeader = ({ title, icon, link, className }: IProps) => {
  return (
    <header className={classNames("flex items-center", className)}>
      ...
      {link && (
        <Link href={link} className="flex items-center mr-6 group">
          <span className="text-[#4F4F4F] font-medium text-sm ml-1 group-hover:text-[#C2123C] transition-color duration-150">مشاهده همه</span>
          <Icon name="arrow-left-2" size={16} color="black" className="group-hover:fill-[#C2123C] transition-color duration-150" />
        </Link>
      )}
    </header>
  )
